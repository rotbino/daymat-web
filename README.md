...
## ۱) ایدهٔ آی مچ در یک نگاه

**آی مچ یک بازار عمده‌فروشی B2B است**: جایی که کسب‌وکارها (نه مصرف‌کننده‌های عادی) کالای خود را با قیمت عمده عرضه می‌کنند، قیمت‌گیری می‌کنند و با هم شبکه می‌سازند.
...
بازارهای تخصصی یعنی آی مچ می‌تواند در هر بازو، **تعداد و تخصص برندها/کالاها/فروشندگان را محدود و کنترل‌شده نگه دارد** — یک «بازوی مصالح» تمیز و حرفه‌ای، نه یک دیوار آشفتهٔ عمومی. دسته‌بندی برندها (بخش ۵) دقیقاً همین زیرساخت است: از این به بعد می‌شود در هر بازو فقط برندهای مرتبط با آن حوزه را پذیرفت.

---

## ۴) دو محصول آی مچ: کاتالوگ قیمت و بازوی خرید
...
- مخاطبین: ماژول `contact` دفترچهٔ تلفن دو-گروهی با سینک و تطبیق اعضای آی مچ دارد (`UserContact`).
...
- ✅ 🤝 مچینگ خودکار تامین‌کننده — «تامین‌کننده‌های این کالا در آی مچ» زیر هر قلم فعالِ بازوی خرید؛ دعوت با یک لمس (`supplier-suggestions`).
- ✅ 🧾 پیش‌فاکتور — فروشنده می‌فرستد، خریدار داخل آی مچ تایید می‌کند؛ تایید = معامله موفق ثبت می‌شود و `saleStatus` پیشنهاد خودکار «sold».
...
- ✅ 🛡️ سیگنال اعتماد — «عضو آی مچ از …» و «N معاملهٔ موفق» در هدر بازوی فروش عمومی.
