...
import { useArms, useActiveCatalog, useCreditBalance } from '@/lib/api/apiHooks';
...
export function AdCreationGate({ children }: { children: React.ReactNode }) {
    ...
    const { activeAdsCount, totalAdsCount } = useMemo(() => {
        const now = new Date();
        const armAds = (catalog?.ads || []).filter((ad: any) => ad.armId === currentArm?.id);
    ...
    }, [catalogLoading, catalog, armsLoading, isMember, currentSlug, router, redirected]);

    // ─── در حال بررسی شرایط ───
    if (!isAuthenticated || catalogLoading || armsLoading || creditLoading || !catalog || !isMember) {
        return (
