...
import { useCataloges, useCreateCatalog, useArms } from '@/lib/api/apiHooks';
...
export default function JoinArmModal({ isOpen, onClose }: JoinArmModalProps) {
    ...
    const { data: cataloges, isLoading: catalogesLoading, refetch: refetchCataloges } = useCataloges();
    ...
    useEffect(() => {
        if (isOpen && cataloges && cataloges.length > 0) {
            setSelectedCatalogId(cataloges[0].id);
            setShowQuickRegister(false);
        } else if (isOpen && cataloges && cataloges.length === 0) {
    ...
    const handleJoinWithCatalog = async (catalogId: string) => {
    ...
    const handleQuickRegister = async (e: React.FormEvent) => {
        ...
        try {
            const newCatalog = await createCatalogMutation.mutateAsync({
                name: quickForm.name,
            ...
            if (newCatalog?.id) {
                await handleJoinWithCatalog(newCatalog.id);
            }
        } catch (error: any) {
    ...
    const hasCataloges = cataloges && cataloges.length > 0;
                    ...
                    {catalogesLoading ? (
                        ...
                        <form onSubmit={handleQuickRegister} className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    placeholder={labels['catalog.name.placeholder'] || 'مثال: شرکت پخش آرمانی'}
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-semibold text-gray-700 dark:text-gray-200 block">
                                    {labels['catalog.shortDescription.label'] || 'توضیح کوتاه'}
                                </label>
                                <input
                                    type="text"
                                    value={quickForm.shortDescription}
                                    onChange={(e) => setQuickForm(prev => ({ ...prev, shortDescription: e.target.value }))}
                                    placeholder={labels['catalog.description.placeholder'] || 'مثال: تولید انواع بلوک'}
                            ...
                            <div className="space-y-1.5">
                                ...
                                >
                                    {CATALOG_TYPES.map(b=> <option value={b.value}>{b.label}</option>)}
                                </select>
                            </div>
                        ...
                        <>
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {cataloges.map((biz: any) => (
                                    <button
                                        key={biz.id}
                                        onClick={() => setSelectedCatalogId(biz.id)}
                                        className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all ${
                                            selectedCatalogId === biz.id
                            ...
                            {/* دکمه پیوستن */}
                            <button
                                onClick={() => selectedCatalogId && handleJoinWithCatalog(selectedCatalogId)}
                                disabled={!selectedCatalogId || isJoining}
