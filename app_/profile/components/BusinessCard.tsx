...
import {CATALOG_TYPE_LABELS} from "@/lib/api/data-types";
...
const CATALOG_TYPE_COLORS: Record<string, string> = {
...
export default function CatalogCard({
    ...
    const logoUrl = catalog?.logoUrl || null;
    ...
    const typeLabel = CATALOG_TYPE_LABELS[catalog?.type] || catalog?.type || '';
    const typeColorClass = CATALOG_TYPE_COLORS[catalog?.type] || CATALOG_TYPE_COLORS.other;
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="w-16 h-16 rounded-xl bg-surface-container-high dark:bg-gray-800 border border-outline dark:border-gray-700 flex items-center justify-center overflow-hidden relative">
                    {logoUrl ? (
                        <Image
                            src={logoUrl}
                            alt={catalog.name || 'لوگوی کسب‌وکار'}
                            width={64}
                            height={64}
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                            onClick={() => router.push(`/catalog/edit/${catalog.id}`)}
            ...
            {isRejected && (
                <div className="mt-3 space-y-2 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg">
                    ...
                    <p className="text-[10px] text-on-surface-variant dark:text-gray-400">
                        {catalog.latestVerification?.notes ? `دلیل: ${catalog.latestVerification.notes}` : 'با پشتیبان خود تماس بگیرید.(09196421264).'}
                    </p>
