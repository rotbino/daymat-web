...
import { useActiveCatalog, useCreditBalance, useCataloges, useCatalog, useArms } from '@/lib/api/apiHooks';
import { AppHeader, AppFooter } from '@/app_/components';
import { setUser } from '@/lib/store/slices/authSlice';
import { EditProfileModal } from '@/app_/profile/components/EditProfileModal';
...
import CatalogCard from '@/app_/profile/components/CatalogCard';
import CatalogAdsList from '@/app_/profile/components/CatalogAdsList';
import CreditsCard from '@/app_/profile/components/CreditsCard';
import TipsList from '@/app_/profile/components/TipsList';
import CatalogList from '@/app_/profile/components/CatalogList';
...
function CatalogNotMemberCard({ armName, catalogName, onJoin }: {
    armName: string;
    catalogName: string;
    onJoin: () => void;
}) {
...
export default function ProfilePage() {
    ...
    const { data: activeCatalog, isLoading: activeLoading, refetch } = useActiveCatalog();
    ...
    const { data: catalogesList } = useCataloges();
    const { data: selectedCatalog, isLoading: catalogLoading, refetch: refetchCatalog } = useCatalog(selectedCatalogId || '');

    const hasTemporaryPassword = user?.temporaryPassword === true;
    const hasCatalog = !!activeCatalog;
    ...
    const isSelectedCatalogMember = useMemo(() => {
        if (!userArms || !currentSlug || !selectedCatalogId) return false;
        return userArms.some(
            (a: any) =>
                a.slug === currentSlug &&
                a.catalogId === selectedCatalogId &&
                a.status === 'active'
        );
    }, [userArms, currentSlug, selectedCatalogId]);

    const isSelectedCatalogPending = useMemo(() => {
        if (!userArms || !currentSlug || !selectedCatalogId) return false;
        return userArms.some(
            (a: any) =>
                a.slug === currentSlug &&
    ...
    const isSelectedCatalogRejected = useMemo(() => {
        if (!userArms || !currentSlug || !selectedCatalogId) return false;
        return userArms.some(
            (a: any) =>
                a.slug === currentSlug &&
    ...
    const rejectionReasonForSelectedCatalog = useMemo(() => {
        if (!userArms || !currentSlug || !selectedCatalogId) return null;
    ...
    const shouldShowWallet = isSelectedCatalogMember;

    useEffect(() => {
        if (activeCatalog && !selectedCatalogId) {
            setSelectedCatalogId(activeCatalog.id);
        }
    }, [activeCatalog, selectedCatalogId]);

    const completionPercentage = useMemo(() => {
        const b = selectedCatalog;
        if (!b) return 0;
    ...
    const isPendingVerify = selectedCatalog?.verificationStatus === 'pending';
    const isRejectedVerify = selectedCatalog?.verificationStatus === 'rejected';
    const isApproved = selectedCatalog?.verificationStatus === 'approved';
    const currentTier = selectedCatalog?.verificationTier || 'none';
    ...
    const totalAds = selectedCatalog?.totalAdsCount || 0;
    const activeAdsCount = selectedCatalog?.activeAdsCount || 0;
    const expiredAdsCount = selectedCatalog?.expiredAdsCount || 0;
    ...
    if ((activeLoading || armsLoading) && !selectedCatalog) {
        return (
    ...
    const membershipGate = (
        <>
            {isSelectedCatalogMember ? (
                ...
                />
            ) : isSelectedCatalogPending ? (
                <PendingMembershipCard />
            ) : isSelectedCatalogRejected ? (
                <RejectedMembershipCard
                    reason={rejectionReasonForSelectedCatalog}
                    onReapply={handleReapply}
                    isSubmitting={reapplying}
                />
            ) : isUserMemberOfArm ? (
                <CatalogNotMemberCard
        ...
        <div className="min-h-screen bg-gradient-to-br from-surface via-surface to-surface-container-low/30 dark:from-gray-950 dark:via-gray-950 dark:to-gray-900/30">
            ...
            <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-24">
                ...
                <div className="hidden lg:grid lg:grid-cols-3 gap-6 mb-8">
                    <div className="lg:col-span-1 space-y-6">
                        <ProfileHeader user={user} catalog={selectedCatalog} isArmOwner={isArmOwner} isSystemAdmin={isSystemAdmin} onEditClick={() => setIsEditModalOpen(true)} />
                        {shouldShowWallet && (
                        ...
                        {!hasCatalog ? (
                            <div className="bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5 dark:from-primary/10 dark:via-primary/15 dark:to-primary/10 border-2 border-primary/30 dark:border-primary/20 rounded-2xl p-6">
                                <div className="flex items-center justify-between gap-4">
                                    ...
                                    <button onClick={() => router.push('/catalog/register')} className="px-6 py-2.5 bg-primary text-on-primary rounded-xl text-sm font-medium">ثبت کسب‌وکار</button>
                                </div>
                            </div>
                        ) : (
                            <>
                    ...
                    {!hasCatalog ? (
                        <div className="bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5 dark:from-primary/10 dark:via-primary/15 dark:to-primary/10 border-2 border-primary/30 dark:border-primary/20 rounded-2xl p-6">
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                ...
                                <button onClick={() => router.push('/catalog/register')} className="px-6 py-2.5 bg-primary text-on-primary rounded-xl text-sm">ثبت کسب‌وکار</button>
                            </div>
                        </div>
                    ) : (
                        <>
                            <CatalogList selectedId={selectedCatalogId} onSelect={(id) => setSelectedCatalogId(id)} />
                            {selectedCatalog && (
                                <CatalogCard
                                    catalog={selectedCatalog}
                                    completionPercentage={completionPercentage}
            ...
            {hasCatalog && selectedCatalog && (
                <VerificationModal
                    ...
                    catalogName={selectedCatalog?.name}
                    currentLevel={selectedCatalog?.verificationTier as any || 'none'}
