...
import { useCatalogBySlug, useCatalogStats } from '@/lib/api/apiHooks';
import {
...
export default function CatalogDashboardPage() {
    ...
    const { data: catalog, isLoading: catalogLoading } = useCatalogBySlug(slug);
    const { data: stats, isLoading: statsLoading } = useCatalogStats(catalog?.id || '');

    // ✅ بررسی مالکیت
    const isOwner = catalog?.owner?.id === user?.id;
            ...
            <div className="sticky top-0 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur border-b border-gray-200/60 dark:border-gray-800">
                <div className={cn(WRAP, 'flex items-center gap-3 py-3')}>
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-gray-400">{catalog.name}</p>
                    </div>
                </div>
            </div>
            ...
            <div className={cn(WRAP, 'py-6 space-y-6')}>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border p-5">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 shrink-0">
                            {catalog.logoUrl ? (
                                <Image src={catalog.logoUrl} alt={catalog.name} width={56} height={56} className="object-cover w-full h-full" unoptimized />
                            ) : (
                                <div className="w-full h-full grid place-items-center"><Building2 className="w-6 h-6 text-gray-400" /></div>
                            )}
                        </div>
                        <div className="flex-1 min-w-0">
                            <h2 className="text-base font-extrabold">{catalog.name}</h2>
                            <p className="text-xs text-gray-400 mt-0.5">
                                {[catalog.city, catalog.type].filter(Boolean).join(' · ')}
                            </p>
                        </div>
                        <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl border p-4">
                        ...
                        <p className="text-sm">{catalog.city || '—'}</p>
                        {catalog.address && <p className="text-xs text-gray-400 mt-1">{catalog.address}</p>}
                    </div>
                    <div className="bg-white dark:bg-gray-900 rounded-2xl border p-4">
                        <h3 className="text-xs font-bold text-gray-400 mb-2 flex items-center gap-1.5">
                        ...
                        <p className="text-sm">
                            {catalog.createdAt
                                ? new Intl.DateTimeFormat('fa-IR', { year: 'numeric', month: 'long' }).format(new Date(catalog.createdAt))
                                : '—'}
                        </p>
                    </div>
                </div>
