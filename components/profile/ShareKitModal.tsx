...
import {
    X, Copy, MessageCircle, Send, Share2, QrCode, Check, Download, Loader2, Printer,
} from 'lucide-react';
...
interface Props {
    ...
    logoUrl?: string;
    /** مسیر پایهٔ صفحهٔ عمومی — پیش‌فرض ریشهٔ سایت (کاتالوگ فروش)؛ اعلام خرید: /inquiries */
    basePath?: string;
    /** نوع محصول — متن‌ها بر همین اساس تنظیم می‌شود (مخاطب کاتالوگ = مشتری؛ مخاطب اعلام خرید = تامین‌کننده) */
...
const COPY_BY_KIND = {
    ...
    inquiry: {
        title: 'کیت اشتراک‌گذاری اعلام خرید',
        intro: 'این صفحه وقتی تامین‌کننده می‌آورد که لینک دستش برسد. لینک را برای تامین‌کننده‌های واتساپی‌ات بفرست یا در گروه‌های خرید و تلگرام به اشتراک بگذار. QR چاپی هم برای تابلوی خریدت هست.',
        shareText: (name: string) => `اعلام خرید «${name}» — اگه تامین‌کننده‌ای، قیمت بده:`,
...
export default function ShareKitModal({ open, onClose, catalogName, slug, logoUrl, basePath = '', kind = 'catalog' }: Props) {
    ...
    const downloadSimpleQr = () => {
        const qr = getQrCanvas('dm-print-qr');
        if (!qr) return;
        try {
