...
interface Props {
    ...
    catalogId?: string;
    /** 📋 شناسهٔ صفحه اعلان نیاز — ذخیره در metadata صفحهٔ خرید (قرینهٔ کاتالوگ) */
    inquiryId?: string;
...
export default function VisitCardModal({ open, onClose, catalogName, slug, logoUrl, phone, description, catalogId, inquiryId, basePath = '', savedSpec, onSaved }: Props) {
    ...
    const saveSpec = async () => {
        ...
        try {
            ...
            // 📋 مقصد ذخیره — کاتالوگ فروش یا صفحه اعلان نیاز (هر دو در metadata.visitCard)
                ...
                <div className="p-4 space-y-3.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <button type="button" onClick={saveSpec} disabled={(!catalogId && !inquiryId) || saving}
                                title={inquiryId ? 'ذخیرهٔ طرح روی صفحه اعلان نیاز' : catalogId ? 'ذخیرهٔ طرح روی کاتالوگ' : 'شناسه ندارد'}
