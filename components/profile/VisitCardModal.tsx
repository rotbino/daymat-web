...
interface Props {
    ...
    catalogId?: string;
    /** 📋 شناسهٔ اعلام خرید — ذخیره در metadata اعلام خرید (قرینهٔ کاتالوگ) */
    inquiryId?: string;
    /** مسیر پایهٔ صفحهٔ عمومی روی QR/کارت — پیش‌فرض ریشه؛ اعلام خرید: /inquiries */
    basePath?: string;
...
export default function VisitCardModal({ open, onClose, catalogName, slug, logoUrl, phone, description, catalogId, inquiryId, basePath = '', savedSpec, onSaved }: Props) {
    ...
    const saveSpec = async () => {
        ...
        try {
            ...
            // 📋 مقصد ذخیره — کاتالوگ فروش یا اعلام خرید (هر دو در metadata.visitCard)
        ...
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center bg-black/50 animate-in fade-in duration-200" onClick={onClose}>
            <div onClick={(e) => e.stopPropagation()}
                ...
                <div className="p-4 space-y-3.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {/* 💾 ذخیره — مشخصات کارت روی کاتالوگ/اعلام خرید می‌ماند و در تب انتشار دیده می‌شود */}
                        <button type="button" onClick={saveSpec} disabled={(!catalogId && !inquiryId) || saving}
                                title={inquiryId ? 'ذخیرهٔ طرح روی اعلام خرید' : catalogId ? 'ذخیرهٔ طرح روی کاتالوگ' : 'شناسه ندارد'}
