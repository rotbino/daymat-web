...
export default function CatalogHeader({
    ...
    //    موبایل: جمع‌وجور (w-9) تا کنار عنوانِ truncate جا شود — دسکتاپ: همان w-12 سابق
    const EditPencil = ({ onClick, label, className }: { onClick: () => void; label: string; className?: string }) => (
        <button
            ...
            title={label}
            className={cn(
                'w-9 h-9 sm:w-12 sm:h-12 rounded-full grid place-items-center transition-all active:scale-90 flex-shrink-0',
    ...
    return (
        <header className={cn('relative pb-3', isEditMode ? 'pt-1' : 'pt-14')}>
            ...
            {/* ═══ دکمه‌های شناور — در مد ویرایش، زیر نوار می‌آیند
                ✅ موبایل: سوییچر ذخیره‌ها فقط-آیکون (بدون متن) تا با دکمه‌های کناری تداخل نکند */}
            <div className={cn('absolute inset-x-0 z-30', isEditMode ? 'top-20' : 'top-4')}>
                <div className={cn(WRAP, 'flex justify-between gap-1.5')}>
                    {/* بازگشت */}
                    <button onClick={onBack} aria-label="بازگشت"
                            className="p-2.5 rounded-full bg-white/80 dark:bg-gray-900/70 backdrop-blur border border-gray-200/70 dark:border-white/10 shadow-md hover:scale-105 active:scale-95 transition-transform flex-shrink-0">
                    ...
                    {/* ═══ سوئیچر بازوهای فروشی ذخیره شده — وسط؛ برای همه کاربران لاگین‌شده (مالک و بازدیدکننده — خواستهٔ مالک)
                        موبایل: فقط Bookmark + شمارش + فلش — بدون متن تا ردیفِ شناور نشکند */}
                    {isAuthenticated && (
                        <div className="flex min-w-0 justify-center items-center mb-1" ref={dropdownRef}>
                            <button
                                onClick={() => setShowSavedDropdown(!showSavedDropdown)}
                                className="inline-flex items-center gap-1 px-2 py-2 sm:px-4 rounded-full text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors"
                            >
                                {/* 🖊 نشانک — موبایل و دسکتاپ هر دو (موبایل فقط-آیکون است) */}
                                <Bookmark className="w-3.5 h-3.5" />

                                {/* ✅ متن فقط دسکتاپ — موبایل جمع‌وجور فقط-آیکون */}
                            ...
                            {showSavedDropdown && (
                                <div className="absolute top-full mt-1 w-72 max-w-[calc(100vw-2rem)] bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/70 dark:border-gray-700 shadow-xl overflow-hidden z-50">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                        ...
                        <button onClick={onSaveToggle}
                                className={cn(
                                    'p-2.5 rounded-full backdrop-blur border shadow-md hover:scale-105 active:scale-95 transition-transform flex-shrink-0',
                                    isSaved
                            ...
                            <div className="flex items-center gap-2.5 mt-3">
                                {ownerAvatar ? (
                                    <Image src={ownerAvatar} alt={catalog.owner?.fullName || ''} width={80} height={80}
                                           className="rounded-full object-cover w-14 h-14 sm:w-16 sm:h-16 shrink-0" unoptimized />
                                ) : (
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-gray-100 dark:bg-gray-800 grid place-items-center shrink-0"><User className="w-7 h-7 text-gray-400" /></div>
                                )}
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                        <div className="min-w-0">
                                            <p className="text-xs font-bold truncate">{catalog.owner?.fullName || 'پروفایل من'}</p>
                                            <p className="text-[11px] text-gray-400 mt-0.5 truncate">{catalog.owner?.bio || 'فروشنده'}</p>
                                        </div>
                                ...
                                <button onClick={onContact}
                                        className="shrink-0 inline-flex items-center gap-1 px-3.5 py-2 rounded-lg bg-primary text-white text-[11px] font-bold active:scale-95 transition">
                                    <Phone className="w-3.5 h-3.5" />تماس
                                </button>
                            </div>
