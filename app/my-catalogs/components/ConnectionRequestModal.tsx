...
//     کسب‌وکارها → «درخواست قیمت» (مشتری ثبت می‌شود؛ تایید با صاحب کسب‌وکار)
...
export default function ConnectionRequestModal({ open, onClose, catalogId, canAssign, sellers }: Props) {
    ...
    const sendBuyerRequest = async (b: any) => {
        ...
        try {
            ...
            toast.success((res?.message || 'درخواست قیمت ارسال شد — در انتظار تایید صاحب کسب‌وکار') + quotaSuffix(res?.quota));
            markSent(`biz-${b.id}`);
            refreshQuota();
        } catch (e: any) {
                        ...
                        return (
                            <div key={key} className="flex items-center gap-3 p-3 rounded-xl border border-outline-variant/30 dark:border-gray-800">
                                ...
                                {busyId === item.id ? (
                                    ...
                                    >
                                        {isBiz && <><ShoppingBasket className="w-3.5 h-3.5" /> درخواست قیمت</>}
