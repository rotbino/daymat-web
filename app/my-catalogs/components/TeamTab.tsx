 ...
 *  [آواتار] [نام + کسب‌وکار] [نقش بیزینسی: فروشنده/ویزیتور/مشتری/مشتری من] [نقش سیستمی: مالک/مدیر کاتالوگ/خودم]
...
import {
    ...
    Loader2, Search, Hourglass, History, LogOut, MoreVertical, ArrowLeftRight, Phone,
} from 'lucide-react';
...
const EVENT_LABEL: Record<string, string> = {
    seller_requested: 'درخواست عضویت در لِین فروش داد',
    seller_approved: 'به‌عنوان فروشنده تایید شد',
...
export default function TeamTab({ catalogId }: Props) {
    ...
    const ordered = [...staff, ...sellers, ...activeCustomers]
        .map((m: any) => ({
            ...m,
        ...
        .sort((a: any, b: any) => {
            const rank = (m: any) => (m.isOwner ? 0 : m.isAdmin ? 1 : m.__isMe ? 2 : m.sellerStatus === 'active' ? 3 : m.__isMyCustomer ? 4 : 5);
            return rank(a) - rank(b);
        });

    const bizBadge = (m: any) => {
        if (m.__isMyCustomer) return { text: 'مشتری من', cls: 'bg-primary text-on-primary' };
        if (m.customerStatus === 'active') return { text: 'مشتری', cls: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300' };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className={cn(CARD_CLS, 'p-4 flex items-center gap-3')}>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                        {ordered.length.toLocaleString('fa-IR')} عضو · {activeCustomers.length.toLocaleString('fa-IR')} مشتری فعال
                        {canManage && pendingSellers.length > 0 && ` · ${pendingSellers.length.toLocaleString('fa-IR')} درخواست جدید`}
                    </p>
                </div>
            ...
            {/* در انتظار تایید مدیر (مالک کاتالوگ) */}
            {canManage && (pendingSellers.length > 0 || pendingCustomers.length > 0) && (
                <div className={cn(CARD_CLS, 'p-4 border-amber-400/40')}>
                    <p className="font-bold text-sm text-amber-700 dark:text-amber-300 mb-3 flex items-center gap-2">
                        ...
                        در انتظار تایید مدیر (مالک کاتالوگ) — {(pendingSellers.length + pendingCustomers.length).toLocaleString('fa-IR')} درخواست
                    </p>
                    <div className="space-y-2">
                            ...
                            <div key={s.id} className="flex items-center gap-2.5 p-3 rounded-xl bg-amber-50/60 dark:bg-amber-900/10">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
                                        {s.business?.name || '—'} · {s.phone}
                                    </p>
                                    <p className="text-[11px] text-amber-600 dark:text-amber-400">در انتظار تایید مدیر (مالک کاتالوگ)</p>
                                </div>
                                    ...
                                    <div className="flex items-center gap-1 flex-shrink-0" onClick={() => { setMenuFor(null); setApproveFor(null); }}>
                                        <div className="relative">
                                            <button
                                                ...
                                                title="تایید با تعیین نقش بیزینسی"
                                            >
                        ...
                        {pendingCustomers.map((c) => (
                            <div key={c.id} className="flex items-center gap-2.5 p-3 rounded-xl bg-amber-50/60 dark:bg-amber-900/10">
                                <Avatar url={c.customerBusiness?.logoUrl || c.avatarUrl} name={c.customerBusiness?.name || c.fullName} size={36} />
                                <div className="flex-1 min-w-0">
                                    <p className="font-bold text-sm text-gray-900 dark:text-gray-100 truncate">{c.customerBusiness?.name || '—'}</p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 truncate">ثبت: {faDate(c.joinedAt)}</p>
                                    <p className="text-[11px] text-amber-600 dark:text-amber-400">در انتظار تایید صاحب کسب‌وکار</p>
                                </div>
                                ...
                                {busy === `rmc-${c.id}` && <Loader2 className="w-4 h-4 animate-spin flex-shrink-0" />}
                            </div>
                        ))}
                    </div>
                </div>
