...
// ✅ دو دکمهٔ ساخت جدا برای هر نوع — صفحه درخواست خرید با bizId دیپ‌لینک می‌شود
...
export function CatalogLinksCard({ catalogs, inquiries = [], businessId }: {
    ...
    /** صفحه‌های خرید متصل به این کسب‌وکار (Inquiry با businessId) */
    inquiries?: any[];
    ...
    const subtitle = salesCount + buyCount === 0
        ...
        : [
              ...
              buyCount > 0 ? `${buyCount.toLocaleString('fa-IR')} صفحه درخواست خرید` : null,
          ].filter(Boolean).join(' · ');
    ...
    return (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-outline-variant/50 dark:border-gray-700 p-4 sm:p-5 space-y-3">
            ...
            {salesCount === 0 && buyCount === 0 ? (
                <div className="rounded-xl border border-dashed border-outline-variant/50 p-4 flex flex-col items-center gap-2 text-center">
                    ...
                    <p className="text-[11px] text-on-surface-variant leading-4">
                        کاتالوگ فروش برای نمایش محصولات، صفحه درخواست خرید برای استعلام قیمت از تامین‌کننده‌ها.
                    </p>
                </div>
            ) : (
                <div className="space-y-2">
                    ...
                    {/* صفحه‌های خرید — کنار فروش، با رنگ کهربایی محصول دوم */}
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 min-w-0">
                                    ...
                                    <span className="text-[9px] font-bold text-amber-600/90 dark:text-amber-400/90 whitespace-nowrap flex-shrink-0">(صفحه درخواست خرید)</span>
                                </div>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    {w.status && (
                            ...
                            <button
                                ...
                                aria-label="کپی لینک صفحه درخواست خرید"
                            ...
                            <Link
                                ...
                                target="_blank"
                                aria-label="مشاهده صفحه درخواست خرید"
                                className="w-8 h-8 rounded-lg grid place-items-center text-on-surface-variant/60 hover:text-amber-600 hover:bg-amber-500/10 active:scale-90 transition-all flex-shrink-0"
                            >
                                <ExternalLink className="w-3.5 h-3.5" />
                            </Link>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
                ...
                >
                    <Plus className="w-3.5 h-3.5" /> صفحه درخواست خرید
                </Link>
            </div>
        </div>
    );
