...
// پنل مدیریت صفحه اعلان نیاز — قرینهٔ کنسول کاتالوگ فروش (/my-catalogs):
...
// ✅ تب «تامین‌کنندگان»: شبکهٔ خرید↔فروش — دعوت/تایید تامین‌کننده‌های صفحه اعلان نیاز
'use client';
...
export default function MyInquiriesPage() {
    ...
    // «صفحه اعلان نیاز کارنت» — پرسیست؛ با رفرش هم سرجاش می‌ماند
    ...
    useEffect(() => {
        document.title = 'پنل صفحه اعلان نیاز | دیمت';
    }, []);
    ...
    const toggleStatus = async () => {
        ...
        try {
            ...
            toast.success(next === 'open' ? 'صفحه اعلان نیاز باز شد' : 'صفحه اعلان نیاز بسته شد');
        } catch (e: any) {
            toast.error(e?.response?.data?.message || 'تغییر وضعیت ناموفق بود');
        }
    };
    ...
    const removeCatalog = async () => {
        if (!currentRow || !window.confirm('این صفحه اعلان نیاز برای همیشه حذف شود؟')) return;
        try {
            ...
            <main className="mx-auto max-w-3xl px-4 md:pt-6">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-4 rounded-3xl border-2 border-dashed border-brand-amber-tint bg-white px-6 py-16 text-center dark:bg-gray-900">
                        ...
                        <div>
                            <h3 className="text-lg font-black">هنوز صفحه اعلان نیازی نساختی</h3>
                    ...
                    <>
                        ...
                        <div className="pt-4">
                            {!currentInquiryId || (!detail && detailLoading) ? (
                                ...
                                <div className="grid place-items-center rounded-3xl border border-stone-100 bg-white py-14 text-center dark:border-gray-800 dark:bg-gray-900">
                                    ...
                                    <p className="mt-3 text-sm font-bold text-stone-400">صفحه اعلان نیاز پیدا نشد</p>
                                </div>
                            )}
                        </div>
                        ...
                        {/* مدال واحدهای اختصاصی صفحه اعلان نیاز */}
                        <UnitSettingsModal
                            isOpen={unitsOpen}
