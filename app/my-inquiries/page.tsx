...
// پنل مدیریت صفحه درخواست قیمت — قرینهٔ کنسول کاتالوگ فروش (/my-catalogs):
//   انتخاب کسب‌وکار در /inquiries/new انجام می‌شود و کاربر مستقیم به همین پنل می‌آید؛
//   اقلام قلم‌به‌قلم از همین‌جا اضافه می‌شوند (هر بار یک کالا + تیک درخواست قیمت).
//   تب‌ها: اقلام | پیشنهادها | تامین‌کنندگان | تنظیمات | انتشار — سوییچر دو-محصولی بالای پنل.
// ✅ تب «تامین‌کنندگان»: شبکهٔ خرید↔فروش — دعوت/تایید تامین‌کننده‌های صفحه درخواست قیمت
'use client';
...
export default function MyInquiriesPage() {
    ...
    // «صفحه درخواست قیمت کارنت» — پرسیست؛ با رفرش هم سرجاش می‌ماند
    ...
    useEffect(() => {
        document.title = 'پنل صفحه درخواست قیمت | دیمت';
    }, []);
    ...
    const toggleUrgent = async (item: InquiryItem) => {
        ...
        try {
            ...
            toast.success(item.urgent ? 'درخواست قیمت این قلم تمام شد' : 'درخواست قیمت فعال شد — تامین‌کننده‌ها قیمت می‌دن');
        } catch (e: any) {
    ...
    const removeCatalog = async () => {
        if (!currentRow || !window.confirm('این صفحه درخواست قیمت برای همیشه حذف شود؟')) return;
        try {
            ...
            <main className="mx-auto max-w-3xl px-4 md:pt-6">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-4 rounded-3xl border-2 border-dashed border-brand-amber-tint bg-white px-6 py-16 text-center dark:bg-gray-900">
                        ...
                        <div>
                            <h3 className="text-lg font-black">هنوز صفحه درخواست قیمتی نساختی</h3>
                    ...
                    <>
                        ...
                        <div className="pt-4">
                            {!currentInquiryId || (!detail && detailLoading) ? (
                                ...
                                <div className="grid place-items-center rounded-3xl border border-stone-100 bg-white py-14 text-center dark:border-gray-800 dark:bg-gray-900">
                                    ...
                                    <p className="mt-3 text-sm font-bold text-stone-400">صفحه درخواست قیمت پیدا نشد</p>
                                </div>
                            )}
                        </div>
                        ...
                        {/* مدال واحدهای اختصاصی صفحه درخواست قیمت */}
                        <UnitSettingsModal
                            isOpen={unitsOpen}
                            ...
                            title="واحدهای صفحه درخواست قیمت"
                            showQtyFields={false}
                        />
