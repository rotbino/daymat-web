...
// تب انتشار پنل کاتالوگ خرید — لینک + اشتراک‌گذاری (واتساپ/تلگرام/سیستمی) + وضعیت دیوار
'use client';
...
export default function PublishTab({ slug, id, title, visibility }: {
    ...
    const shareText = `کاتالوگ خرید «${title}» — اگه تامین‌کننده‌ای، قیمت بده: ${url}`;
    ...
    const nativeShare = async () => {
        try {
            if (navigator.share) {
                await navigator.share({ title: `کاتالوگ خرید — ${title}`, text: shareText, url });
            } else {
                await copy();
            }
        } catch { /* کاربر لغو کرد */ }
    ...
    return (
        <div className="space-y-3">
            ...
            <motion.section
                ...
                <h2 className="mb-3 text-[12px] font-black text-stone-400 dark:text-gray-500">لینک کاتالوگ خرید</h2>
            ...
            <motion.section
                ...
                <div className="flex gap-2">
                    ...
                    <a href={`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`کاتالوگ خرید «${title}»`)}`} target="_blank" rel="noreferrer" className={`${btn} text-sky-500 hover:text-sky-600`}>
