...
// صفحه «سرور در دسترس نیست» — وقتی اینترنت وصل است ولی بک‌اند یا دیتابیس پاسخ نمی‌دهد.
// رفتار:
//   • اول پروب فعال اینترنت (navigator.onLine قابل‌اعتماد نیست) —
//     اگر اینترنت واقعاً قطع بود → صفحه آفلاین (offline.html)
//   • هر ۵ ثانیه GET /health را پینگ می‌کند (مهلت ۵ ثانیه)
//   • 200 → هدایت خودکار به صفحه مقصد (ذخیره‌شده در sessionStorage)
...
const STATUS_TEXT: Record<Status, { title: string; sub: string }> = {
    checking: { title: 'در حال بررسی اتصال…', sub: 'یه لحظه، وضعیت سرور آی مچ را چک می‌کنیم.' },
    offline: {
        title: 'اتصال اینترنت قطع است',
        sub: 'وای‌فای یا دیتای موبایل خود را بررسی کنید. به‌محض وصل شدن، خودکار به صفحه موردنظر برمی‌گردید.',
    },
    backend: {
        title: 'سرور در دسترس نیست',
        sub: 'ارتباط با سرور آی مچ برقرار نیست. به‌محض برقراری ارتباط، به صفحه موردنظر هدایت می‌شوید.',
    },
    db: {
        title: 'سرور در دسترس نیست',
        sub: 'سرور بالا است اما پایگاه داده پاسخ نمی‌دهد. به‌محض برقراری ارتباط، به صفحه موردنظر هدایت می‌شوید.',
    },
};
...
export default function ServerUnavailablePage() {
    ...
    const check = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            if (!online) {
                // اینترنت قطع است → صفحه آفلاین (طبق قاعده: قطعی اینترنت = صفحه آفلاین)
