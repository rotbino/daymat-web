...
interface Props {
    ...
    /** ✅ عمومی‌شده: ذخیره‌ساز سفارشی (مثلاً صفحه درخواست خرید → PATCH استعلام) — پیش‌فرض: کانفیگ کاتالوگ فروش */
    saveFn?: (units: { unitId: string; containsQty?: number; qtyIsFixed?: boolean }[]) => Promise<any>;
    /** ✅ عنوان مدال — پیش‌فرض: واحدهای اختصاصی کاتالوگ */
    title?: string;
    /** ✅ نمایش فیلدهای بسته‌بندی (تعداد داخلش / ثابت) — صفحه درخواست خرید اینها را ندارد */
