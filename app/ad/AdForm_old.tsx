...
import {
    useCreateAd, useActiveCatalog, useCreditBalance,
...
export function AdForm({ adId, onSuccess }: AdFormProps) {
    ...
    useEffect(() => {
        const fetchCounts = async () => {
            if (!catalog || !currentSlug) {
                setLoadingAdsCounts(false);
                return;
            }
            try {
                const now = new Date();
                const allCatalogAds = catalog.ads || [];
                const armAds = allCatalogAds.filter((ad: any) => ad.armId === currentArm?.id);
        ...
        fetchCounts();
    }, [catalog, currentSlug, currentArm]);

    const isLoading = catalogLoading || creditLoading || (isEditMode && adLoading) || loadingAdsCounts;
    ...
    useEffect(() => {
        ...
        } else if (catalog?.cityCode && !hasSingleCity) {
            setFormData(prev => ({
                ...prev,
    ...
    }, [catalog, hasSingleCity, singleCityData, isEditMode]);
